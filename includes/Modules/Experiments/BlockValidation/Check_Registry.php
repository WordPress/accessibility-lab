...
/**
 * Holds every registered validation check, grouped by scope.
 *
 * Checks live in memory for the duration of the request; nothing is persisted
 * here. The level a check declares at registration can be overridden
 * downstream through the `validation_api_check_level` filter — see
 * `resolve_level()`.
 */
final class Check_Registry {
	 ...
	 */
	public function register_namespace( string $namespace, array $args ): void { // phpcs:ignore Universal.NamingConventions.NoReservedKeywordParameterNames -- `namespace` is this API's documented parameter name.
		if ( '' === $namespace ) {
			return;
		}
	...
	/**
	 * Every registered check, grouped by scope.
	 *
	...
	public function resolve_level( array $check ): string {
		...
		$context = array(
			...
			// phpcs:ignore WordPress.DB.SlowDBQuery.slow_db_query_meta_key -- array key in a filter context, not a query arg.
	...
	/**
	 * Fill in defaults for a caller-supplied check definition.
	 *
	 * `name` is the slug used for keys and JS filters; `title` is the human
	 * label shown to admins and falls back to the slug so it stays optional.
	 *
